'''
    主要思路:
        ...
        3. 客户端收到后会从服务端的缓存文件夹中下载对应的图片并打印到聊天界面,
           同样是在接收后进行判断进行操作
    继续第3步: 将缓存的图片贴到聊天框, 继续在fileGet函数添加代码
    遇到的问题: 如果pho(打开图片存储的变量)
                和窗口作用域不一样就只能显示空白图片
                解决参考:http://blog.csdn.net/u011320646/article/details/12845761
'''
...
pho = tkinter.PhotoImage(file = pictureName)
# 从图片服务端的缓存文件夹中下载图片到客户端缓存文件夹中
def fileGet(name):
    global pho   # 如果pho和窗口作用域不一样就只能显示空白图片
    PORT3 = 50009
    ...
    # 打开图片然后贴到聊天框
    pictureName = './客户端图片缓存/' + name
    pho = tkinter.PhotoImage(file = pictureName)
    listbox.image_create(tkinter.END, image=pho)
