'''
    开始加入发送表情功能
    主要思路: 通过发送标记然后在接收的时候进行判断是文本还是表情进行打印
    
    创建表情字典, 将标记与图片一一对应
    然后创建表情按钮 -> 按钮显示图片
    参考: https://zhidao.baidu.com/question/119814361.html
    按钮的边框也去掉
    参考: https://zhidao.baidu.com/question/505279124.html
...
dic = {'aa**':'./表情/捂脸.png', 'bb**':'./表情/奸笑.png',
       'cc**':'./表情/皱眉.png', 'dd**':'./表情/机智.png'}
# 四个按钮, 使用全局变量, 方便创建和销毁
b1 = ''
b2 = ''
b3 = ''
b4 = ''
# 将图片打开存入变量中
p1 = tkinter.PhotoImage(file = dic['aa**'])
p2 = tkinter.PhotoImage(file = dic['bb**'])
p3 = tkinter.PhotoImage(file = dic['cc**'])
p4 = tkinter.PhotoImage(file = dic['dd**'])
